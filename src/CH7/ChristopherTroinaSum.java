...
public class ChristopherTroinaSum {
    ...
    public static void arrayCreate(Scanner scan, File file){
        ...
        int[][] add=new int[maxparam+1][125];
    ...
    public static void addition(int[][] add){
        for(int i=0;i<125;i++){
            ...
            for(int ii=0;ii<add.length;ii++) {
                sum += add[ii][i];
                if(sum<10)
                    add[add.length-1][i]=sum;
                else{
                    String sumst=Integer.toString(sum);
                    add[add.length-1][i-1]+=Character.getNumericValue(sumst.charAt(0));
                    add[add.length-1][i]=Character.getNumericValue(sumst.charAt(1));
                }
            }
        }
        if(add[add.length-1][add[0].length-1]==0)
            System.out.println("0");
        int index=0;
        while(add[add.length-1][index]==0)
            index++;
        for(int i=0;i<125;i++)
            System.out.print(add[add.length-1][i]);
        System.out.println();
    }
}